...
public abstract class AbsCallback<T> implements Converter<T> {
     ...
     * 对返回数据进行操作的回调， UI线程
     */
    public abstract void onSuccess(T t, Call call, Response response, int id);

    /**
     * 缓存成功的回调,UI线程
     */
    public void onCacheSuccess(T t, Call call, int id) {
    }
     ...
     * 缓存失败的回调,UI线程
     */
    public void onCacheError(Call call, Exception e, int id) {
    }

    /**
     * 网络失败结束之前的回调
     */
    public void parseError(Call call, Exception e, int id) {
    }

    /**
     * 请求网络结束后，UI线程
     */
    public void onAfter(T t, Exception e, int id) {
     ...
     */
    public void upProgress(long currentSize, long totalSize, float progress, long networkSpeed, int id) {
    }
     ...
     * @param networkSpeed 当前下载的速度 字节/秒
     */
    public void downloadProgress(long currentSize, long totalSize, float progress, long networkSpeed, int id) {
    }
}