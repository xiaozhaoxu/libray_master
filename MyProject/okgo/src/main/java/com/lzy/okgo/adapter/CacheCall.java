...
public class CacheCall<T> implements Call<T> {
    ...
    public void execute(AbsCallback<T> callback) {
        ...
        //请求执行前UI线程调用
        mCallback.onBefore(baseRequest, baseRequest.getId());
        //请求之前获取缓存信息，添加缓存头和其他的公共头
        if (baseRequest.getCacheKey() == null)
            baseRequest.setCacheKey(HttpUtils.createUrlFromParams(baseRequest.getBaseUrl(), baseRequest.getParams()
                    .urlParamsMap));
        ...
        if (cacheMode != CacheMode.NO_CACHE) {
            ...
            if (cacheEntity != null && cacheEntity.checkExpire(cacheMode, baseRequest.getCacheTime(), System
                    .currentTimeMillis())) {
        ...
        rawCall.enqueue(new okhttp3.Callback() {
            ...
            public void onFailure(okhttp3.Call call, IOException e) {
                if (e instanceof SocketTimeoutException && currentRetryCount < baseRequest.getRetryCount()) {
                    ...
                    mCallback.parseError(call, e, baseRequest.getId());
    ...
    private void handleCache(Headers headers, T data) {
        ...
        CacheEntity<T> cache = HeaderParser.createCacheEntity(headers, data, baseRequest.getCacheMode(), baseRequest
                .getCacheKey());
        if (cache == null) {
            //服务器不需要缓存，移除本地缓存
     ...
     */
    @SuppressWarnings("unchecked")
    private void sendFailResultCallback(final boolean isFromCache, final okhttp3.Call call, final okhttp3.Response
            response, final Exception e) {
        final CacheMode cacheMode = baseRequest.getCacheMode();
        ...
        OkGo.getInstance().getDelivery().post(new Runnable() {
            ...
            public void run() {
                if (isFromCache) {
                    mCallback.onCacheError(call, e, baseRequest.getId());             //缓存失败回调 （UI线程）
                    if (cacheMode == CacheMode.DEFAULT || cacheMode == CacheMode.REQUEST_FAILED_READ_CACHE) {
                        mCallback.onAfter(null, e, baseRequest.getId());              //请求结束回调 （UI线程）
                    }
                } else {
                    mCallback.onError(call, response, e, baseRequest.getId());        //请求失败回调 （UI线程）
     ...
     * 成功回调，发送到主线程
     */
    private void sendSuccessResultCallback(final boolean isFromCache, final T t, final okhttp3.Call call, final
    okhttp3.Response response) {
        ...
        OkGo.getInstance().getDelivery().post(new Runnable() {
            ...
            public void run() {
                if (isFromCache) {
                    mCallback.onCacheSuccess(t, call, baseRequest.getId());           //缓存成功回调 （UI线程）
                    if (cacheMode == CacheMode.DEFAULT || cacheMode == CacheMode.REQUEST_FAILED_READ_CACHE ||
                            cacheMode == CacheMode.IF_NONE_CACHE_REQUEST) {
                        mCallback.onAfter(t, null, baseRequest.getId());              //请求结束回调 （UI线程）
                    }
                } else {
                    mCallback.onSuccess(t, call, response, baseRequest.getId());      //请求成功回调 （UI线程）
                    mCallback.onAfter(t, null, baseRequest.getId());                  //请求结束回调 （UI线程）
                }
            }
        });
