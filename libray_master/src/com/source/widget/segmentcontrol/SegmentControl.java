package com.source.widget.segmentcontrol;
...
public class SegmentControl extends View {
    ...
    int dir_up_num=0;//文字没有上下居中,竖直方向减去一个值

    public void setDir_up_num(int dir_up_num) {
        this.dir_up_num = dir_up_num;
    }
    ...
    public void onDraw(Canvas canvas){
        ...
        if(mTexts != null && mTexts.length > 0){
            for(int i = 0; i < mTexts.length; i++){
                ...
                //draw texts
                canvas.drawText(mTexts[i], mCacheBounds[i].left + (mSingleChildWidth - mTextBounds[i].width()) / 2, mCacheBounds[i].top + (( mTextBounds[i].height()+mSingleChildHeight-dir_up_num) / 2), mPaint);
            }
        }
